---
title: 'camels-attrs: A Python Package for Extracting CAMELS-like Catchment Attributes for Any Area Draining to a USGS Streamflow Gauging Station in the United States'
tags:
  - Python
  - hydrology
...
authors:
  ...
  - name: Venkatesh Merwade
    ...
    affiliation: 1
affiliations:
  - name: Lyles School of Civil and Construction Engineering, Purdue University, West Lafayette, IN, USA
    index: 1
...
Understanding watersheds is fundamental to managing water resources, predicting floods, and assessing environmental change. Scientists characterize watersheds using attributes such as average watershed elevation, soil type, vegetation cover, and climate patterns. The Catchment Attributes and Meteorology for Large-sample Studies (CAMELS) dataset [@addor2017camels; @newman2015development] provides standardized attributes for 671 watersheds across the United States and has become a useful resource in hydrology research.

`camels-attrs` is an open-source Python package (MIT Licensed) that extends the CAMELS methodology to extract data for areas draining to any United States Geological Survey (USGS) stream gauge location in the country. Using USGS station identifier as input, the package automatically delineates the upstream watershed boundary according to drainage basins and extracts over 70 standardized catchment attributes related to topography, climate, soil, vegetation, geology, and hydrology. The package also retrieves daily hydrometeorological forcing data (precipitation, temperature, solar radiation, wind speed, humidity) for user-specified time periods. Visualization tools produce publication-ready watershed maps and multi-gauge comparison figures. The package is available on PyPI and GitHub.
...
This work was partially supported by funding from the Lyles School of Civil and Construction Engineering at Purdue University, and from the US National Science Foundation (grants 2230092 and 2118329). We acknowledge the developers of the HyRiver suite of Python packages upon which this package builds, and the agencies—including USGS, NOAA, and NASA—that provide open access to hydrological and geospatial data.
